...
def render(ckpt, save_dir, rays):
    epoch_idx = ckpt.find('epoch=')
    epoch = ckpt[epoch_idx, ckpt[epoch_idx:].find('-')]
    model = nerf_model.NeRFNetwork.load_from_checkpoint(ckpt).to(device)
    nerf_helpers.generate_360_view_synthesis(model, save_dir, epoch, N=rays)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('-s', '--save_dir', type=str, default='./recons/', help='where to save the resulting gif')
